    ...
    def __post_init__(self) -> None:
        ...
        if self.band_power_config is None:
            self.band_power_config = BandPowerConfig(
                sampling_rate=self.sampling_rate, window_seconds=self.window_seconds
            )
...
class BandPowerExtractor:
    ...
    def extract(self, data: np.ndarray) -> np.ndarray:
        ...
        for band_idx, (band_name, (f_low, f_high)) in enumerate(
            self.config.bands.items()
        ):
            ...
            if not np.any(band_mask):
                warnings.warn(
                    f"No frequencies in band {band_name} ({f_low}-{f_high} Hz)"
                )
                continue
...
class CSP:
    ...
    def fit(self, X: np.ndarray, y: np.ndarray) -> "CSP":
        ...
        try:
            ...
            raise ValueError(
                "CSP fitting failed. Try increasing regularization."
            ) from e
        ...
        component_idx = np.concatenate(
            [np.arange(n_components), np.arange(n_channels - n_components, n_channels)]
        )
